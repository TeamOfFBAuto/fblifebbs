...
+ (NSString *)transformString:(NSString *)originalStr WithImageWidth:(float)theWidth WithHeight:(float)theHeight
{
    ...
    if ([array_emoji count]) {
        ...
        for (NSString *str in array_emoji) {
            ...
            NSString * path = [[NSBundle mainBundle] pathForResource:i_transCharacter ofType:nil];
...
+ (void)creatAttributedText:(NSString *)o_text Label:(OHAttributedLabel *)label OHDelegate:(id<OHAttributedLabelDelegate>)delegate WithWidht:(float)aWidth WithHeight:(float)aHeight WithLineBreak:(BOOL)isBreak
{
    label.automaticallyAddLinksForType = 0;//不让系统自动检测网址链接
    
    NSMutableArray *httpArr = [OHLableHelper addHttpArr:o_text];
    NSString *text = [OHLableHelper transformString:o_text WithImageWidth:aWidth WithHeight:aHeight];
...
+ (float)returnHeightAttributedText:(NSString *)o_text Label:(OHAttributedLabel *)label WithWidht:(float)aWidth WithHeight:(float)aHeight
{
    label.automaticallyAddLinksForType = 0;//不让系统自动检测网址链接
